...
class Display3DImage:
    ...
    def __init__(self,image):
        ...
        plt.title('# of slices = '+str(self.slice_number))
        plt.show()
    ...
    def drawImage(self, slice_direction, slice_number):
        if (slice_direction == 0):   # yz plane
            self.slice_image = self.image[slice_number,0:,0:]
        if (slice_direction == 1):   # xz plane
            self.slice_image = self.image[0:,slice_number,0:]
        if (slice_direction == 2):   # xy plane
            self.slice_image = self.image[0:,0:,slice_number]
        
        plt.imshow(self.slice_image)
        #plt.show()
        plt.title('# of slices = '+str(slice_number))
        plt.draw()
    ...
    def on_press(self,event):
        ...
        self.slice_number = int(min(max_slice_num, max(0, self.slice_number + event.step)))
...
def SaveMhd(datafilename, data, order='F'):
    ...
    try:
        ...
        f.write('ElementDataFile = '+os.path.basename(datafilename)+'\n')
        f.close()
...
class Calc_kloc:
    ...
    @staticmethod
    def Calc_gradient_predict(filename_GIRF,filename_GIRFx, seq_G, f_cut=20, amp_cut=1.2, offset=32768, GIRF=None, GIRF_x=None):
        """
        GIRF予測のgradientを計算 (1D, 2D, and 3D)
        f_cut : [kHz]
        GIRF :  1D double complex
        GIRF_x　: [kHz] 1D double
        """
        if filename_GIRF != '':
        ...
        elif len(seq_G.shape) == 2:                       #2D
            for i in range(seq_G.shape[1]):
                print(i,end=",")
                gradient_predict[:,i]= Calc_Goutput(seq_G[:,i]-offset, gradient_x, GIRF, GIRF_x, f_cut=f_cut, amp_cut=amp_cut)            
        else:
                gradient_predict[:]= Calc_Goutput(seq_G[:]-offset, gradient_x, GIRF, GIRF_x, f_cut=f_cut, amp_cut=amp_cut)            
    ...
    def Calc_gradient_predict_test(filename_GIRF,filename_GIRFx, seq_G, f_cut=20, amp_cut=1.2, offset=32768, GIRF=None, GIRF_x=None):
        """
        仕様変更2018/12/26
        GIRF予測のgradientを計算 (1D, 2D, and 3D)
        seq_G : G波形 [横軸はus刻み]
        ...
        #GIRF[int(len(GIRF)/2)] =   (GIRF[int(len(GIRF)/2-1)] +   GIRF[int(len(GIRF)/2+1)] ) /2 
        
        gradient_predict = np.zeros(np.shape(seq_G))
        #拡張
        N = seq_G.shape[0]
        gradient_x = np.arange(-N, N,dtype="float64")*1e-3 #[ms]
        seq_G_large = np.zeros((N*2), dtype=seq_G.dtype)
        if len(seq_G.shape) > 2:    #3D
            for j in range(seq_G.shape[2]):
                for i in range(seq_G.shape[1]):
                    print(i,j,end=",")
                    seq_G_large[N:] = seq_G[:,i,j]
                    gradient_predict_large = Calc_Goutput(seq_G_large-offset, gradient_x, GIRF, GIRF_x, f_cut=f_cut, amp_cut=amp_cut)
                    gradient_predict[:,i,j]= gradient_predict_large[N:]
        elif len(seq_G.shape) == 2:                       #2D
            for i in range(seq_G.shape[1]):
                print(i,end=",")
                seq_G_large[N:] = seq_G[:,i]
                gradient_predict_large= Calc_Goutput(seq_G_large-offset, gradient_x, GIRF, GIRF_x, f_cut=f_cut, amp_cut=amp_cut)            
                gradient_predict[:,i]= gradient_predict_large[N:]            
        else:
                #拡張
                seq_G_large[N:] = seq_G[:]
                #seq_G_large[N:N+N] = seq_G[:]
                gradient_predict_large = Calc_Goutput(seq_G_large-offset, gradient_x, GIRF, GIRF_x, f_cut=f_cut, amp_cut=amp_cut)            
                gradient_predict = gradient_predict_large[N:]
                #gradient_predict = gradient_predict_large[N:N+N]
        return gradient_predict+offset
